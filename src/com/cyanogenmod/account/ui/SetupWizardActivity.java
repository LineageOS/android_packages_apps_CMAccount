...
public class SetupWizardActivity extends Activity implements SetupDataCallbacks {
    ...
    private void removeSetupPage(final Page page, boolean animate) {
        ...
        final int indexOfPageToRemove = mPageList.indexOf(page);
        if (animate) {
        ...
        // If we have moved beyond the page to remove,
        // make sure we aren't on the last page
        Page currentPage =
                (position == mPageList.size() || indexOfPageToRemove < position)
                ? mPageList.get(position - 1)
                : mPageList.get(position);
