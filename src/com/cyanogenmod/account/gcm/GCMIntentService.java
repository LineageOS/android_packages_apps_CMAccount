...
public class GCMIntentService extends IntentService implements Response.Listener<Integer>, Response.ErrorListener {
    ...
    private void handleSecureMessage(final GCMessage message) {
        ...
        if (!verifySignature(message.getPayload(), message.getSignature())) {
            sendFailureMessage();
        ...
        if (plaintextMessage.getSequence() != 1) {
            Log.w(TAG,  "Sequence " + plaintextMessage.getSequence() + " is invalid for keyId " + encryptedMessage.getKeyId());
            return;
        }
    ...
    private boolean verifySignature(String payload, String remoteSignature) {
        String localSignature = EncryptionUtils.HMAC.getSignature(mHmacSecret, payload);
