...
public class GCMIntentService extends IntentService implements Response.Listener<Integer>, Response.ErrorListener {
    ...
    private void handleSecureMessage(final GCMessage message) {
        ...
        ECPoint remotePublicKey = message.getPublicKey();
        BigInteger privateKey = getPrivateKey(keyId);
        if (privateKey == null) {
            sendFailureMessage();
            return;
        }
    ...
    private boolean verifyPublicKeySignature(GCMessage message, byte[] hmacSecret) {
        ECPoint publicKey = message.getPublicKey();
        String remoteSignature = message.getPublicKeySignature();
        String signatureBody = CMAccountUtils.encodeHex(publicKey.getX()) + ":" + CMAccountUtils.encodeHex(publicKey.getY());
    ...
    private BigInteger getPrivateKey(String keyId) {
        ...
        cursor.close();
        return new BigInteger(CMAccountUtils.decodeHex(privateKeyString));
    }
